...
def lambda_handler(event, context):
    ...
    if _terms is not None:
        terms = [Term(term=term.term, type=term.type, evidence=term.evidence, explanation=term.explanation, document_id=document_id, longterm_id=longterm_id, meta=meta) for term in _terms if term is not None]
