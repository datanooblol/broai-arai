...
def create_term_search_trigger_and_index():
    with engine.connect() as conn:
        # Run outside transaction block to allow isolated errors
        trans = conn.begin()
        try:
            conn.execute(text("""
                CREATE TRIGGER term_search_vector_update
                BEFORE INSERT OR UPDATE ON term
                FOR EACH ROW EXECUTE FUNCTION
                tsvector_update_trigger(search_vector, 'pg_catalog.english', term);
            """))
        except DBAPIError as e:
            if "already exists" in str(e):
                print("🔁 Trigger already exists, skipping.")
        ...
        try:
            conn.execute(text("""
                CREATE INDEX IF NOT EXISTS idx_term_search_vector
                ON term USING GIN (search_vector);
            """))
        except DBAPIError as e:
            print("❌ Index creation failed.")
            trans.rollback()
...
def initialize_memories():
    get_extensions()
    create_db_tables()
    create_term_search_trigger_and_index()
